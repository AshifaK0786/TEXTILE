    ...
    <Container>
      <AnimatedContainer>
        ...
        {(summary || uploadResults.length > 0) && (
          <StatsGrid>
            <StatCard>
              <Card.Title>✅ Delivered Amount</Card.Title>
              <Card.Text style={{ color: '#48bb78', fontSize: '1.8rem' }}>
                {formatCurrency(summary?.deliveredProfit || calculateUploadTotals(uploadResults)?.deliveredAmount || 0)}
              </Card.Text>
            </StatCard>
            <StatCard>
              <Card.Title>🔄 RPU Amount</Card.Title>
              <Card.Text style={{ color: '#e53e3e', fontSize: '1.8rem' }}>
                {formatCurrency(summary?.rpuProfit || calculateUploadTotals(uploadResults)?.rpuAmount || 0)}
              </Card.Text>
            </StatCard>
            <StatCard>
              <Card.Title>📦 RTO Amount</Card.Title>
              <Card.Text style={{ color: '#ff6347', fontSize: '1.8rem' }}>
                {formatCurrency(summary?.rtoProfit || calculateUploadTotals(uploadResults)?.rtoAmount || 0)}
              </Card.Text>
            </StatCard>
            <StatCard>
                ...
                return totals && (
                  <div style={{ fontSize: '0.9rem', textAlign: 'right' }}>
                    <div style={{ color: '#667eea', fontWeight: '600' }}>
                      📦 {totals.totalProducts} products | 💰 {formatCurrency(totals.totalAmount)}
                    </div>
                    <div style={{ fontSize: '0.8rem', color: '#666', marginTop: '0.2rem' }}>
                      Profit: {formatCurrency(totals.totalProfit)} ({totals.profitMargin}%)
                    </div>
                  </div>
                );
              })()
              ...
              {(() => {
                ...
                return totals && (
                  <span style={{ fontSize: '0.8rem', fontWeight: 'normal', marginLeft: '1rem', color: '#666' }}>
                    ({totals.totalProducts} products, {formatCurrency(totals.totalAmount)} total)
                  </span>
                );
              })()
              }
            ...
            {(() => {
              ...
              return totals && (
                <div style={{ marginBottom: '1.5rem', padding: '1rem', backgroundColor: '#f8f9fa', borderRadius: '10px' }}>
                  <Row>
                    <Col md={3}>
                      <p style={{ margin: 0, fontSize: '0.9rem', color: '#666' }}>Delivered Amount</p>
                      <p style={{ margin: 0, fontSize: '1.5rem', fontWeight: 'bold', color: '#28a745' }}>
                        {formatCurrency(totals.deliveredAmount)}
                      </p>
                    </Col>
                    <Col md={3}>
                      <p style={{ margin: 0, fontSize: '0.9rem', color: '#666' }}>RPU Amount</p>
                      <p style={{ margin: 0, fontSize: '1.5rem', fontWeight: 'bold', color: '#dc3545' }}>
                        {formatCurrency(totals.rpuAmount)}
                      </p>
                    </Col>
                    <Col md={3}>
                      <p style={{ margin: 0, fontSize: '0.9rem', color: '#666' }}>RTO Amount</p>
                      <p style={{ margin: 0, fontSize: '1.5rem', fontWeight: 'bold', color: '#ff6347' }}>
                        {formatCurrency(totals.rtoAmount)}
                      </p>
                    </Col>
                    <Col md={3}>
