...
export const profitLossAPI = {
  getProfitLoss: (startDate, endDate, includeSales = false) => {
    const params = new URLSearchParams();
    if (startDate) params.append('startDate', startDate);
    if (endDate) params.append('endDate', endDate);
    if (includeSales) params.append('includeSales', 'true');
  ...
  getEntries: (startDate, endDate, limit = 200) => {
    const params = new URLSearchParams();
    if (startDate) params.append('startDate', startDate);
    if (endDate) params.append('endDate', endDate);
    if (limit) params.append('limit', String(limit));
    return api.get(`/profit-loss/entries?${params.toString()}`);
  },
  exportExcel: (data) => {
  ...
  getComboDetails: (sku) => {
    return api.get(`/profit-loss/combo-details/${encodeURIComponent(sku)}`);
  },
  getDebugCombos: () => {
    return api.get('/profit-loss/debug-combos');
  },
  getLatestUploads: (limit = 10) => {
    return api.get(`/profit-loss/uploads/latest?limit=${limit}`);
  },
  deleteUpload: (id) => {
    return api.delete(`/profit-loss/uploads/${id}`);
  },
  testRTOCreation: (data) => {
    return api.post('/profit-loss/test-rto', data);
  },
};
...
export const rtoProductsAPI = {
  ...
  getDebugAll: () => api.get('/rto-products/debug/all'),
};
