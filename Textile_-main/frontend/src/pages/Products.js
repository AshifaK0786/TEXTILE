...
import { productsAPI, vendorsAPI, returnsAPI, categoriesAPI, rtoProductsAPI } from '../services/api';
  ...
  const handleRTOSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        // Create RTO tracking entries for each item
        console.log(`📦 Creating RTO tracking entries for ${rtoFormData.items.length} items`);
        for (const item of rtoFormData.items) {
          if (item.product) {
            try {
              const rtoData = {
                ...
                category: 'RTO',
                quantity: item.quantity,
                reason: rtoFormData.reason || 'Product return',
                notes: `Return ID: ${returnId}, Customer: ${rtoFormData.customerName}`,
                dateAdded: new Date(rtoFormData.returnDate)
              };
              console.log('📝 Creating RTO entry:', rtoData);
              const rtoResponse = await rtoProductsAPI.create(rtoData);
              console.log('✅ RTO entry created:', rtoResponse.data);
            } catch (rtoError) {
              console.error('❌ Error creating RTO tracking entry:', rtoError);
              console.error('RTO Error details:', rtoError.response?.data);
            }
          }
        }
  ...
  const handleRPUSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        // Create RPU tracking entries for each item
        console.log(`📦 Creating RPU tracking entries for ${rpuFormData.items.length} items`);
        for (const item of rpuFormData.items) {
          if (item.product) {
            try {
              const rpuData = {
                productId: item.product,
                category: 'RPU',
                quantity: item.quantity,
                reason: rpuFormData.reason || 'Product pickup',
                notes: `Return ID: ${returnId}, Customer: ${rpuFormData.customerName}`,
                dateAdded: new Date(rpuFormData.returnDate)
              };
              console.log('📝 Creating RPU entry:', rpuData);
              const rpuResponse = await rtoProductsAPI.create(rpuData);
              console.log('✅ RPU entry created:', rpuResponse.data);
            } catch (rpuError) {
              console.error('❌ Error creating RPU tracking entry:', rpuError);
              console.error('RPU Error details:', rpuError.response?.data);
            }
          }
        }
