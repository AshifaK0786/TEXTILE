...
const RTOProducts = () => {
  ...
  useEffect(() => {
    const checkDatabase = async () => {
      try {
        console.log('🔍 RTOProducts: Checking database on mount...');
        const response = await rtoProductsAPI.getDebugAll();
        console.log('📊 Database check result:', response.data.stats);
      } catch (err) {
  ...
  const fetchProducts = async (forceRefresh = false) => {
    ...
    setError('');
    try {
      console.log(`🔄 Fetching ${activeTab} products${forceRefresh ? ' (FORCE REFRESH)' : ''}...`);
      
      // First get all products without filters to see what's in DB
      const allResponse = await rtoProductsAPI.getDebugAll();
      console.log('📊 All RTO/RPU in DB:', allResponse.data);
      ...
      console.log(`✅ Fetched ${response.data.length} ${activeTab} products:`, response.data);
      setProducts(response.data);
      
    } catch (err) {
      console.error('❌ Error fetching RTO/RPU products:', err);
                ...
                <td>
                  ...
                  {product.source && (
                    <small style={{ display: 'block', color: '#666' }}>
                      {product.source === 'returns' ? '📋 Returns' : '📊 Upload'}
                    </small>
                  )}
                </td>
