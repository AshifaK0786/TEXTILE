...
// DELETE uploaded profit sheet and associated ProfitLoss entries
router.delete('/:id', async (req, res) => {
  try {
    ...
    // Delete associated ProfitLoss entries
    const deletedEntries = await ProfitLoss.deleteMany({ uploadId: req.params.id });
    console.log(`🗑️ Deleted ${deletedEntries.deletedCount} ProfitLoss entries for upload ${req.params.id}`);

    // Delete the uploaded profit sheet
    await UploadedProfitSheet.findByIdAndDelete(req.params.id);

    res.json({ 
      message: 'Uploaded profit sheet and associated entries deleted successfully',
